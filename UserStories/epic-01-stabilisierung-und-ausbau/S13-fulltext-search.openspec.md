...
- **AC:**
  ...
  - [x] Der Index funktioniert im Aspire- und docker-compose-Setup: Beide sind auf ein neues Image `docker/mssql-fts` (Aspire: `.WithDockerfile("../docker/mssql-fts")`, docker-compose: `build: ./docker/mssql-fts`) statt des Standard-`mssql/server`-Images umgestellt, das per `apt-get install mssql-server-fts` die fehlende Komponente nachinstalliert. In dieser Sandbox mangels Internetzugang zu `packages.microsoft.com` nicht baubar; **verifiziert über GitHub-Actions-CI-Run [#23](https://github.com/MaikPeters1511/AudioTranscription/actions/runs/36062366206)** auf Commit `65a7fc2` (mit echtem Internetzugang): Der Image-Build lief durch, `Backend build & test` war grün mit **301/301** Tests (3 Min. 7 Sek. Laufzeit, deutlich länger als ein reiner Unit-Test-Lauf — ein Beleg dafür, dass die FTS-Testklassen tatsächlich liefen, nicht übersprungen wurden).
...
- **AC (TDD):**
  - [x] Integrationstests gegen eine echte DB (Testcontainers) für Treffer, keine Treffer, Sonderzeichen und Paginierung: `SearchIntegrationTests` (10 Tests) decken alle geforderten Fälle ab (Treffer in `RawTranscript`/Segment/Variante, keine Treffer, drei FTS-Sonderzeichen-Payloads inkl. eines SQL-Injection-Versuchs, fehlende Query → 400, Paginierung, Stemming). In dieser Sandbox mangels FTS-fähigem Testcontainers-Image nicht ausführbar; **grün in GitHub-Actions-CI-Run [#23](https://github.com/MaikPeters1511/AudioTranscription/actions/runs/36062366206)** (siehe T2). Die reine Snippet-/Offset-Logik (`SearchSnippetBuilder`, 7 Tests, mutationsgetestet) läuft unabhängig davon auch hier grün.
...
- [x] Die Suche nach einem Wort aus einer deutschen Aufnahme findet sie auch in gebeugter Form (Stemming), sofern die gewählte Technologie das unterstützt. `FREETEXT` mit `LANGUAGE 1031` (Deutsch) unterstützt das; der Test `Search_SupportsGermanStemming` prüft das explizit und lief grün in GitHub-Actions-CI-Run [#23](https://github.com/MaikPeters1511/AudioTranscription/actions/runs/36062366206) (in dieser Sandbox mangels Internetzugang nicht ausführbar, siehe T2).
