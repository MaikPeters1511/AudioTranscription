...
| Betrieb (Aspire/docker-compose) | Bereits vorhandener SQL-Server-Container, **aber geprüft und korrigiert:** Anders als zunächst angenommen ist Full-Text Search im offiziellen Linux-Container-Image `mcr.microsoft.com/mssql/server:2022-latest` **nicht** enthalten (`SERVERPROPERTY('IsFullTextInstalled')` liefert `0`). Es ist ein separates apt-Paket (`mssql-server-fts`), das auf dem Image nachinstalliert werden muss — siehe Konsequenzen. | Neuer Container/Service in beiden Umgebungen |
...
- **Full-Text-Search-Komponente fehlt im Standard-Image:** `mcr.microsoft.com/mssql/server:2022-latest` hat Full-Text Search **nicht** vorinstalliert (per Test widerlegt, siehe Tabelle oben). Ein eigenes Image `docker/mssql-fts/Dockerfile` installiert das apt-Paket `mssql-server-fts` auf dem Basis-Image; Aspire (`AppHost/Program.cs`, `.WithDockerfile("../docker/mssql-fts")`) und `docker-compose.yml` bauen daraus statt das Standard-Image direkt zu verwenden.
- **`mssql-server-fts` braucht ein eigenes apt-Repository:** Ein erster Dockerfile-Entwurf scheiterte auch mit funktionierendem Internetzugang (bestätigt über die GitHub-Actions-CI dieses Repos, nicht nur in dieser Sandbox) mit `E: Unable to locate package mssql-server-fts` — das allgemeine, im Basis-Image bereits vertraute `packages.microsoft.com/…/prod`-Repository enthält dieses Paket nicht. Es liegt in einem eigenen, versionsspezifischen Repository, das erst per `curl https://packages.microsoft.com/config/ubuntu/22.04/mssql-server-2022.list` registriert werden muss, bevor `apt-get update && apt-get install mssql-server-fts` funktioniert (siehe [microsoft/mssql-docker#422](https://github.com/microsoft/mssql-docker/issues/422)). `docker/mssql-fts/Dockerfile` macht das jetzt.
- **Nicht in dieser Sandbox testbar:** Der eigentliche `curl`-Aufruf gegen `packages.microsoft.com` scheitert aus dieser Entwicklungsumgebung heraus an einem TLS-Zertifikatsfehler (selbstsigniertes Zertifikat des Egress-Proxys) — dieselbe Einschränkungsklasse wie huggingface.co/github.com bei den Whisper- (S08) und Diarisierungs-Modellen (S11). Der `SearchIndexMigrationTests`-Testlauf schlägt hier deshalb beim Image-Build fehl, nicht bei der eigentlichen Migrations- oder Suchlogik.
- **Verifiziert:** GitHub-Actions-CI-Run [#23](https://github.com/MaikPeters1511/AudioTranscription/actions/runs/36062366206) auf Commit `65a7fc2` (echter Internetzugang) hat den korrigierten Dockerfile-Ansatz bestätigt — der Image-Build lief durch, `Backend build & test` war grün mit 301/301 Tests in 3 Min. 7 Sek. (deutlich länger als ein reiner Unit-Test-Lauf, ein Beleg dafür, dass Image-Build und FTS-Tests tatsächlich liefen statt übersprungen zu werden). Der volle Volltextsuche-Pfad (Migration, `SearchIntegrationTests`, deutsches Stemming) ist damit end-to-end bestätigt.
