...
- **Nicht in dieser Sandbox testbar:** Der eigentliche `curl`-Aufruf gegen `packages.microsoft.com` scheitert aus dieser Entwicklungsumgebung heraus an einem TLS-Zertifikatsfehler (selbstsigniertes Zertifikat des Egress-Proxys) — dieselbe Einschränkungsklasse wie huggingface.co/github.com bei den Whisper- (S08) und Diarisierungs-Modellen (S11). Der `SearchIndexMigrationTests`-Testlauf schlägt hier deshalb beim Image-Build fehl, nicht bei der eigentlichen Migrations- oder Suchlogik. Ob der korrigierte Dockerfile-Ansatz in einer Umgebung mit echtem Internetzugang (GitHub-Actions-CI) tatsächlich durchläuft, ist zum Zeitpunkt dieses Commits noch offen und wird über das CI-Ergebnis dieses Pull Requests verifiziert.
