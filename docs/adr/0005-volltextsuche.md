...
- **Nicht in dieser Sandbox testbar:** Das Bauen von `docker/mssql-fts/Dockerfile` braucht Netzwerkzugriff auf `packages.microsoft.com` (apt-Repository für `mssql-server-fts`); dieser Host ist aus dieser Entwicklungsumgebung heraus mit einem TLS-Zertifikatsfehler gesperrt — dieselbe Einschränkungsklasse wie huggingface.co/github.com bei den Whisper- (S08) und Diarisierungs-Modellen (S11). Der `SearchIndexMigrationTests`-Testlauf hier schlägt deshalb beim Image-Build fehl (`E: Unable to locate package mssql-server-fts`), nicht bei der eigentlichen Migrations- oder Suchlogik. In einer Umgebung mit normalem Internetzugang (z. B. der GitHub-Actions-CI dieses Repos) sollte derselbe Build und dieselben Tests durchlaufen; das muss ein Betreiber mit Netzwerkzugriff verifizieren.
